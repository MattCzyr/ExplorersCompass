...
public class ExplorersCompassAngleState extends NeedleAngleState {
	...
	protected float getAngle(ItemStack stack, ClientWorld world, int seed, Entity entity) {
		...
		if (stack.getItem() == ExplorersCompass.EXPLORERS_COMPASS_ITEM) {
			...
			if (compassItem.getState(stack) == CompassState.FOUND) {
				pos = new GlobalPos(world.getRegistryKey(), new BlockPos(compassItem.getFoundStructureX(stack), 0, compassItem.getFoundStructureZ(stack)));
			}
		}
