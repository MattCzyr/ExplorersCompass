	...
	public static void clientInit(FMLClientSetupEvent event) {
		event.enqueueWork(() -> {
			...
			try {
				...
				layers.add(new ExplorersCompassOverlay());
			} catch (IllegalAccessException e) {
				ExplorersCompass.LOGGER.error("Failed to add Explorer's Compass GUI layer");
	...
	public static void constructMod(FMLConstructModEvent event) {
		event.enqueueWork(() -> {
			try {
				...
				idMapper.put(ResourceLocation.fromNamespaceAndPath(ExplorersCompass.MODID, "angle"), ExplorersCompassAngle.MAP_CODEC);
			} catch (IllegalAccessException e) {
				ExplorersCompass.LOGGER.error("Failed to register Explorer's Compass model property");
