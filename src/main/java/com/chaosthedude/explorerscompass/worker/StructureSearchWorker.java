...
public abstract class StructureSearchWorker<T extends StructurePlacement> implements WorldWorkerManager.IWorker {
    ...
    protected Pair<BlockPos, Structure> getStructureGeneratingAt(ChunkPos chunkPos) {
        ...
        for (Structure structure : structureSet) {
            StructureCheckResult result = level.structureManager().checkStructurePresence(chunkPos, structure, ConfigHandler.GENERAL.ignoreExplored.get());
            if (result != StructureCheckResult.START_NOT_PRESENT) {
                if (!ConfigHandler.GENERAL.ignoreExplored.get() && result == StructureCheckResult.START_PRESENT) {
                ...
                if (structureStart != null && structureStart.isValid() && (!ConfigHandler.GENERAL.ignoreExplored.get() || tryAddReference(level.structureManager(), structureStart))) {
    ...
    private boolean tryAddReference(StructureManager manager, StructureStart start) {
        // TODO: accesstransformer for `ChunkGenerator.tryAddReference()`
